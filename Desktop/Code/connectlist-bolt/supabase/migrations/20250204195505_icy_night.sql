/*
  ...
  1. New Tables
    - categories (Base categories for all content types)
    - content_types (Types for people: actor, author, athlete, etc.)
    - movies (Movie information with TMDB-like structure)
    - series (TV series information with TMDB-like structure)
    - people (People information with multiple roles)
    - games (Game information)
    - softwares (Software information)
    - musics (Music information)
    - places (Place information similar to Google Places)
    - books (Book information)
    - companies (Company information)
    - content_people (Junction table for content-people relationships)
    - content_companies (Junction table for content-company relationships)
...
CREATE TABLE people (
  ...
  content_types uuid[] REFERENCES content_types(id),
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
