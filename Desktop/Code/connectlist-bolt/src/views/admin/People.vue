<script setup lang="ts">
import { ref, onMounted } from 'vue'
import { supabase } from '../../lib/supabase'
import { PhPerson, PhPlus, PhPencil, PhTrash } from '@phosphor-icons/vue'
import type { Database } from '../../lib/supabase-types'

interface Person {
  full_name: string
  biography: string | null
  birth_date: string | null
  death_date: string | null
  nationality: string | null
  website: string | null
  social_media: any | null
  profile_image_url: string | null
  content_types: string[]
}

const people = ref<Database['public']['Tables']['people']['Row'][]>([])
const isLoading = ref(true)
const error = ref('')
const isModalOpen = ref(false)

const newPerson = ref({
  full_name: '',
  biography: null,
  birth_date: null,
  death_date: null,
  nationality: null,
  website: null,
  social_media: null,
  profile_image_url: null,
  content_types: []
})

const resetForm = () => {
  newPerson.value = {
    full_name: '',
    biography: null,
    birth_date: null,
    death_date: null,
    nationality: null,
    website: null,
    social_media: null,
    profile_image_url: null,
    content_types: []
  }
}

const handleSubmit = async () => {
  try {
    const { error: insertError } = await supabase
      .from('people')
      .insert(newPerson.value)

    if (insertError) throw insertError

    isModalOpen.value = false
    resetForm()
    loadPeople()
  } catch (err: any) {
    error.value = err.message
  }
}

const loadPeople = async () => {
  try {
    const { data, error: fetchError } = await supabase
      .from('people')
      .select('*')
      .order('created_at', { ascending: false })
    
    if (fetchError) throw fetchError
    
    people.value = data || []
  } catch (err: any) {
    error.value = err.message
  } finally {
    isLoading.value = false
  }
}

onMounted(() => {
  loadPeople()
})
</script>

<template>
  <div>
    <!-- Header -->
    <div class="mb-8">
      <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <PhPerson :size="28" weight="bold" />
          People
        </h1>
        <button 
          @click="isModalOpen = true"
          class="bg-primary text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-primary/90"
        >
          <PhPlus :size="20" weight="bold" />
          Add Person
        </button>
      </div>
      <p class="text-gray-600 mt-1">Manage people and their roles</p>
    </div>

    <!-- Error Message -->
    <div v-if="error" class="mb-6 p-4 bg-red-50 text-red-600 rounded-lg">
      {{ error }}
    </div>

    <!-- People Table -->
    <div class="bg-white rounded-lg shadow overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                Name
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                Roles
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                Nationality
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                Actions
              </th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-if="isLoading">
              <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                Loading people...
              </td>
            </tr>
            <tr v-else-if="people.length === 0">
              <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                No people found
              </td>
            </tr>
            <tr v-for="person in people" :key="person.id" class="hover:bg-gray-50">
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <img 
                    v-if="person.profile_image_url"
                    :src="person.profile_image_url"
                    :alt="person.full_name"
                    class="w-10 h-10 rounded-full object-cover"
                  />
                  <div v-else class="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center">
                    <PhPerson :size="20" class="text-gray-500" weight="bold" />
                  </div>
                  <div class="ml-4">
                    <div class="text-sm font-medium text-gray-900">
                      {{ person.full_name }}
                    </div>
                    <div v-if="person.birth_date" class="text-sm text-gray-500">
                      Born: {{ new Date(person.birth_date).toLocaleDateString() }}
                    </div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex flex-wrap gap-1">
                  <span 
                    v-for="type in person.content_types" 
                    :key="type"
                    class="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800"
                  >
                    {{ type }}
                  </span>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ person.nationality || 'N/A' }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <div class="flex space-x-2">
                  <button 
                    class="p-1 text-gray-400 hover:text-primary rounded-full hover:bg-gray-100"
                    title="Edit person"
                  >
                    <PhPencil :size="20" weight="bold" />
                  </button>
                  <button 
                    class="p-1 text-gray-400 hover:text-red-600 rounded-full hover:bg-gray-100"
                    title="Delete person"
                  >
                    <PhTrash :size="20" weight="bold" />
                  </button>
                </div>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>

    <!-- Add Person Modal -->
    <div v-if="isModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-200">
          <h2 class="text-xl font-semibold text-gray-900">Add New Person</h2>
        </div>

        <form @submit.prevent="handleSubmit" class="p-6 space-y-6">
          <!-- Full Name -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Full Name *</label>
            <input
              v-model="newPerson.full_name"
              type="text"
              required
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary"
            >
          </div>

          <!-- Biography -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Biography</label>
            <textarea
              v-model="newPerson.biography"
              rows="4"
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary"
            ></textarea>
          </div>

          <!-- Birth Date -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Birth Date</label>
            <input
              v-model="newPerson.birth_date"
              type="date"
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary"
            >
          </div>

          <!-- Nationality -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Nationality</label>
            <input
              v-model="newPerson.nationality"
              type="text"
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary"
            >
          </div>

          <!-- Website -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Website</label>
            <input
              v-model="newPerson.website"
              type="url"
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary"
            >
          </div>

          <!-- Profile Image URL -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Profile Image URL</label>
            <input
              v-model="newPerson.profile_image_url"
              type="url"
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary"
            >
          </div>

          <!-- Form Actions -->
          <div class="flex justify-end gap-4 pt-4 border-t border-gray-200">
            <button
              type="button"
              @click="isModalOpen = false"
              class="px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200"
            >
              Cancel
            </button>
            <button
              type="submit"
              class="px-4 py-2 text-white bg-primary rounded-lg hover:bg-primary/90"
            >
              Add Person
            </button>
          </div>
        </form>
      </div>
    </div>
  </div>
</template>